...
if [[ ! -f "${BASE_DATA}" ]]; then
  IMAGE_ROOT=${IMAGE_ROOT:-data/external/lingoqa} TARGET_SIZE="${MAX_SAMPLES}" OUTPUT="${BASE_DATA}" \
    bash scripts/21_prepare_lingoqa_subset.sh "${SOURCE:-data/external/lingoqa/val.parquet}"
fi

if [[ ! -f "${WRONG_DATA}" ]]; then
...
if [[ ! -f "${BLANK_DATA}" ]]; then
  python src/data/make_visual_ablation.py \
    --input "${BASE_DATA}" \
    --output "${BLANK_DATA}" \
    --mode blank_image \
    --blank_image_path outputs/cases/ablation_images/lingoqa_blank.jpg
fi
...
PREFIX="lingoqa_qwen25vl_3b_100_best_spatial_3frame"
