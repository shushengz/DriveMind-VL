...
python src/train_qwen25vl_dpo_lora.py \
  ...
  --train_file "${TRAIN_FILE:-data/processed/lingoqa_preference_v5_from_sft_v2_train_predictions.jsonl}" \
  --output_dir "${OUTPUT_DIR:-outputs/checkpoints/qwen25vl_3b_lingoqa_pref_v5_from_predictions}" \
  --max_pairs "${MAX_PAIRS:-0}" \
  --epochs "${EPOCHS:-1}" \
  --max_steps "${MAX_STEPS:-120}" \
  --save_steps "${SAVE_STEPS:-30}" \
  --gradient_accumulation_steps "${GRAD_ACCUM:-8}" \
  --learning_rate "${LR:-8e-7}" \
