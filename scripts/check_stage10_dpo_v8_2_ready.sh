...
CONFIG_PATH="${CONFIG_PATH:-configs/dpo_v8_2_lingo_smoke.yaml}"
mkdir -p outputs/gpu_stage

STAGE10_MODEL_PATH="$MODEL_PATH" STAGE10_CONFIG_PATH="$CONFIG_PATH" "$PYTHON_BIN" - <<'PY' 2>&1 | tee outputs/gpu_stage/stage10_dpo_v8_2_ready.log
...
model_path = Path(os.environ["STAGE10_MODEL_PATH"])
config_path = Path(os.environ["STAGE10_CONFIG_PATH"])
r3 = "checkpoints/qwen25vl_lora_sft_v3_r3_lingo_smoke/"
preference = Path("data/train/preference_v8_2/preference_v8_2_pairs.jsonl")
audit_path = Path("outputs/data_audit/preference_v8_2_audit.json")
...
if model_path.exists():
    try:
        ...
        config = AutoConfig.from_pretrained(model_path.as_posix(), trust_remote_code=True)
        processor = AutoProcessor.from_pretrained(model_path.as_posix(), trust_remote_code=True)
        add("qwen25vl_local_loadable", True, {"config": config.__class__.__name__, "processor": processor.__class__.__name__})
    except Exception as exc:
        add("qwen25vl_local_loadable", False, str(exc))

adapter = Path(r3)
add("r3_policy_init_adapter", (adapter / "adapter_config.json").exists(), adapter.as_posix())
add("r3_reference_adapter", (adapter / "adapter_config.json").exists(), adapter.as_posix())
add("preference_v8_2_data", preference.exists(), preference.as_posix())
if preference.exists():
    add("preference_v8_2_nonempty", True, sum(1 for line in preference.open(encoding="utf-8") if line.strip()))
if audit_path.exists():
    audit = json.loads(audit_path.read_text(encoding="utf-8"))
    add("preference_v8_2_train_ready", audit.get("train_ready") is True, audit.get("train_ready"))
    add("heldout_leakage_count", audit.get("heldout_leakage_count") == 0, audit.get("heldout_leakage_count"))
    actual_rate = float(audit.get("rejected_from_actual_prediction_rate", 0))
    add("rejected_from_actual_prediction_rate", actual_rate >= 0.80, actual_rate)
else:
    add("preference_v8_2_audit", False, f"missing {audit_path}")

add("dpo_v8_2_config", config_path.exists(), config_path.as_posix())
...
if cfg:
    ...
    add("config_learning_rate", float(cfg.get("learning_rate", "nan")) == 5e-8, cfg.get("learning_rate"))
    add("config_max_steps", int(cfg.get("max_steps", "-1")) == 25, cfg.get("max_steps"))
...
Path("outputs/gpu_stage/stage10_dpo_v8_2_ready.json").write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
