"""Build the Stage 8 held-out comparison table from saved raw predictions."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Aggregate Stage 8 held-out DPO-v8.1 comparisons.")
    ...
    parser.add_argument("--output", default="outputs/final_report/stage8_dpo_v8_1_heldout_main_results.csv")
    parser.add_argument("--warnings_output", default="outputs/final_report/stage8_dpo_v8_1_heldout_main_results_warnings.json")
    ...
    target = Path(args.warnings_output)
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(json.dumps({"warnings": warnings}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
